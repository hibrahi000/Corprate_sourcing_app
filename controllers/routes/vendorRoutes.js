									...
									if (newMaterial === false) {
										mat.findOne({ Category: category }).then((matDoc) => {
											...
											if (mProfile.Material[mMatI].Vendors[0] === undefined) {
												...
												const mailOptionsVendUnsubscirbeNewDel = {
												...
												`,
													html: `Since ${vendorName} requested to be removed from the email chain for material: ${material} in category ${category}, we do not have any vendors that support it so it was removed from the database. <br><br> If the vendor contacts you to undo this change you can always re-add the material in the <em>Modify Material<em> Page in the purchase app. All you will have to do is: <br> 1) search for the vendors name<br>2)Select the category that the material belongs in<br> 3)Add the material ** Spaces should be replaced with dashes and multiple materials should be comma separated AND no spaces before or after the commas <br> 4)Then click save
												...
												const mailOptionsVendUnsubscirbeNew = {
